...
Please use ``Thinslaves-setup --help`` to learn how to use the new features.

.. HINT::

    Converting to JSON: ``Thinslaves-setup convert <instance_name> json``

    Converting to Mongo: ``Thinslaves-setup convert <instance_name> mongo``
 ...
 * ``Thinslaves --version`` will now give you current version of Red (`#2567`_)
 ...
 * ``Thinslaves-setup`` now uses the click CLI library (`#2579`_)
 * ``Thinslaves-setup convert`` now used to convert between libraries (`#2579`_)
